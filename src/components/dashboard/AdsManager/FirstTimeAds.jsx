...
const FirstTimeAds = () => {
	const { createAdCampaign, connectedSocialAccounts } = useStore();
	...
	const handleCreateAd = (platform) => {
		// Check if connected
		// For demo, we check if ANY account is connected, or specifically the selected one
		// If platform is provided, check if that specific one is connected.
		// If "Get Started" (no platform), check if any is connected.
		...
		if (!isConnected) {
			...
			setSelectedPlatform(
				platform || connectedSocialAccounts[0].platform
			);
				...
				<div className='flex flex-col gap-5 w-full md:w-1/2 mb-4 md:mb-0'>
					...
					<div className='flex items-center'>
						<button
							...
							className='px-8 py-2 bg-orange-gradient text-white rounded-xl flex items-center gap-2'
						>
							Get Started
						</button>
					</div>
			...
			>
				...
				>
					{socialAccounts.map((social, index) => (
						<motion.div
							key={index}
							onClick={() => handleCreateAd(social.name)}
